...
{   
    for(uint8_t uIndex=0; uIndex<BUTTON_COUNT; uIndex++)
    {
        ...
        privateLcd.FillRect(1, BUTTON_LENGTH*uIndex+1, BUTTON_WIDTH-2, BUTTON_LENGTH-2);
        
        privateLcd.SetTextColor(LCD_COLOR_GREEN);
        privateLcd.DrawRect(0, (uIndex*BUTTON_LENGTH), BUTTON_WIDTH, BUTTON_LENGTH);
        
        privateLcd.SetTextColor(LCD_COLOR_WHITE);
        privateLcd.DisplayStringAt( 0, uIndex*BUTTON_LENGTH, (uint8_t *) Text[uIndex], LEFT_MODE);
    }
}